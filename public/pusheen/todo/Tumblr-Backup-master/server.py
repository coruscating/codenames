...
class TumblrDownloader(object):
    ...
    def request(self, url, filename=None):
        ...
        try:
            ...
            print("Failed to fetch page: {}".format(url))
            returnValue([])
        else:
            #print "ok?"
    ...
    def request2(self, url, filename=None):
        print "request"
        if filename is None:
            url = safe_format("http://api.tumblr.com/v2/blog/{}/{}", self.url, url)
            url += "&api_key="+API_KEY if "?" in url else "?api_key="+API_KEY
    ...
    def blog_info(self):
        ...
        print info["response"]["blog"]
        if info:
            if info["response"]["blog"]["posts"] > 20000:
                print("Archives limited to 20,000 posts for now. Send Fugi an ask to inquire about an exception.", True)
                shutil.rmtree(self.folder)
            else:
                self.blog = info["response"]["blog"]
                self.avatar_info()
        else:
            print("Not a valid blog. Aborting.", True)
            shutil.rmtree(self.folder)
    ...
    def download_posts(self):
        ...
        for offset in range(0, self.blog["posts"], 20):
            print(safe_format("Fetching Posts ({:d}/{:d})", offset, self.blog["posts"]))
            result = self.request(safe_format("posts?offset={:d}", offset))
    ...
    def download_images(self):
        ...
        for index, image in enumerate(self.image_queue):
            ...
            print(safe_format("Fetching Images ({:d}/{:d})", index+1, images))
            ...
            if scheme in ("http","https"):
                ...
                with open(filename,"wb") as f:
                    f.write(self.request2(url, filename))
...
test2=TumblrUser()
test1=TumblrServer()
test1.download("pusheen.tumblr.com",test2)
